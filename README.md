...
## Political-cartoon crosswalks (guaranteed-PD cartoons) — Wave 3
`crosswalk/cartoons/` — **52 public-domain political cartoons** across four courses (Grade 7: 4, Grade 8: 28, World: 10, Tennessee: 10). Grade 6 is excluded — the ancient/classical era has no political cartoons in the modern sense, so none were invented. Sources are guaranteed PD by category: Reformation woodcuts and early-modern prints via The Met (CC0); the American canon (Franklin's *Join, or Die*, Thomas Nast's Tweed Ring and Reconstruction cartoons, *King Andrew the First*, suffrage cartoons) via the Library of Congress Prints & Photographs Division (pre-1929 published) and Met CC0; British satire (Gillray, *Punch*) via LOC and Met. **212 standards are honestly flagged "none"** — where no PD cartoon fits the standard, the row says so rather than forcing a weak match. All four courses validate at 0 blockers under the Rule 7 validator; `verified:false` for the download agent. This completes the visual-sources phase (maps + charts + cartoons).
